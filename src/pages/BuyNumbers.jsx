...
const BuyNumbers = ({ darkMode }) => {
  ...
  // ---------------- HELPERS ----------------
  const formatNumber = (num) =>
    !num ? "" : String(num).startsWith("+") ? num : `+${num}`;
    ...
    pollOtp.current = setInterval(async () => {
      try {
        ...
        if (res.data?.otp) {
          ...
          setOrderStatus("received");
          clearInterval(pollOtp.current);
          localStorage.removeItem("activeBuySession");
        }
      } catch {}
    }, 2000);
  };
  ...
  // ---------------- RESTORE SESSION ----------------
  useEffect(() => {
    const saved = localStorage.getItem("activeBuySession");
    if (!saved) return;
    ...
    try {
      ...
      const remaining = Math.floor(
        (session.expiryTime - Date.now()) / 1000
      );
      ...
      setTimeLeft(remaining);
      setOrderStatus(session.status || "waiting");

      startOtpPolling(session.orderid);
    } catch {
      localStorage.removeItem("activeBuySession");
    }
  }, []);
    ...
    const fetchServices = async () => {
      ...
      try {
        ...
        const mapped = data.map((s) => {
          const priceObj = s.pricing?.find(
            (p) =>
              String(p.countryID) === String(selectedCountry.ID)
          );
  ...
  // ---------------- BUY ----------------
  const handleBuy = async (service) => {
    if (!selectedCountry) return alert("Select country first");
    if (!service.price) return alert("Unavailable service");
    if (balance < service.price) return alert("Insufficient balance");
    if (orderStatus === "waiting") return alert("Active order exists");

    const previousBalance = balance;
    setBalance((p) => p - service.price);
      ...
      <div className="buy-layout">
        ...
        <div className="otp-panel">
          ...
          {!activeOrder ? (
            ...
            <>
              <h2>{formatNumber(activeOrder.number)}</h2>
              ...
              {orderStatus === "expired" && <p>Expired</p>}
            </>
          )}
        </div>
      </div>
